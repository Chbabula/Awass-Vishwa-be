...
const createItem = async (req, res) => {
  ...
  if (title.length < 5 || title.length > 100) {
    res
      ...
      .json({
        error:
          "title length should be greater than equals to 5 and less equals to 100 characters",
      });
    return;
  }
  if (location.length < 3 || location.length > 100) {
    res
      ...
      .json({
        error:
          "location length should be greater than equals to 3 and less equals to 100 characters",
      });
    return;
  }
  ...
  if (description.length > 1000) {
    res
      .status(400)
      .json({
        error: "description length should be less than 1000 characters",
      });
    return;
  }
